...
export default function ChatInterno({ 
  ...
  // 1. INICIALIZAÇÃO SEGURA DO ABLY E CONTROLE DE PRESENÇA
  useEffect(() => {
    if (!meuId) return;

    // Resgata a chave de forma dinâmica e segura dentro do ciclo de vida
    const ABLY_KEY = import.meta.env.VITE_ABLY_KEY || "";
    if (!ABLY_KEY) {
      console.error("⚠️ [Erro Ably]: VITE_ABLY_KEY não definida no ambiente de produção.");
      return;
    }

    const ably = new Ably.Realtime({
      key: ABLY_KEY,
      clientId: String(meuId)
    });
    ...
    const atualizarPresenca = async () => {
      try {
        ...
        const mapaOnline = {};
        membros.forEach(m => {
          mapaOnline[m.clientId] = true;
        });
        setUsuariosOnline(mapaOnline);
      } catch (err) {
        console.error("Erro ao sincronizar presença no Ably:", err);
      }
    };
    ...
    return () => {
      if (canalPresenca) canalPresenca.presence.leave().catch(() => {});
      if (ably) ably.close();
    };
  }, [meuId, minhaEmpresa, meuNome, minhaFoto]);
  ...
  useEffect(() => {
    const buscarContatos = async () => {
      ...
      try {
        ...
        const filtrados = dados.filter(u => {
          ...
          return String(uId) !== String(meuId) && uEmail?.toLowerCase() !== meuEmail?.toLowerCase();
        });
    ...
    if (minhaEmpresa && meuId) buscarContatos();
  }, [minhaEmpresa, meuId, meuEmail]);

  // 3. SELECIONAR CONVERSA E ABRIR CANAL PRIVADO
  ...
  useEffect(() => {
    ...
    canalSala.subscribe('nova-mensagem', (message) => {
      ...
      setMensagens(prev => {
        const jaExiste = prev.some(m => String(m.id || m.Id) === String(novaMsg.Id || novaMsg.id));
        if (jaExiste) return prev;
        
        return [...prev, {
          id: novaMsg.Id || novaMsg.id,
          salaId: novaMsg.SalaId || novaMsg.salaId,
          remetenteId: novaMsg.RemetenteId || novaMsg.remetenteId,
          texto: novaMsg.Texto || novaMsg.texto,
          dataEnvio: novaMsg.DataEnvio || novaMsg.dataEnvio,
          visualizado: novaMsg.Visualizado || novaMsg.visualizado
        }];
      });

      const msgRemetenteId = novaMsg.RemetenteId || novaMsg.remetenteId;
      if (String(msgRemetenteId) !== String(meuId)) {
        tocarSomNotificacao();
        fetch(`${API_URL}/chat/mensagens/visualizar/${novaMsg.Id || novaMsg.id}`, { method: 'POST' }).catch(() => {});
      }
    });
        ...
        <div style={styles.contatosList}>
          {loadingContatos ? (
            ...
            contatosFiltrados.map(c => {
              ...
              const selecionado = String(contatoAtivoId) === String(cId);
